...
3. Теперь требуется наполнить базу данных таблицами. Для этого откройте другую вкладку в Терминале перейдите в папку api проекта.
4. Перейдите в контейнер crud-api: ```docker exec -it crud-api sh```.
5. Создайте таблицы: ```npx prisma migrate dev```.
6. Можете выйти из контейнера: ```exit```.

## Первоначальная настройка если будите проводить тестирование
1. Перейдите в папку проекта.
2. Запустить Докер (если не запущен) в стандартном режиме: ```docker-compose -f docker-compose-dev.yml up --build```.
3. Для концевого тестирования используется отдельная база данных. Чтобы её создать зайдите в контейнер базы данных: ```docker exec -it crud-bd sh```.
4. Переключитесь на пользователя postgres ```su postgres```
5. Запустите процесс psql: ```psql```
6. Создайте базу данных для тестирования: ```create database crud_test;```
7. Вывод списка баз данных: ```\l```
8. Можно выйти из контейнера дважды или трижды запустив: ```exit```
9. Теперь требуется наполнить тестовую базу данных таблицами. Для этого перейдите в папку api проекта.
4. Перейдите в контейнер crud-api: ```docker exec -it crud-api sh```.
5. Создайте таблицы: ```npx prisma migrate dev --schema=./prisma_test/schema.prisma```.
6. Можете выйти из контейнера: ```exit```.

## Стандартный запуск API
Если вы уже выполнили все шаги из «Первоначальной настройки», то для стандартного запуска API выполните два первых шага из группы «Первоначальная настройка».

## Тестирование API
Если вы уже выполнили все шаги из «Первоначальная настройка если будите проводить тестирование», то для запуска тестов Докер нужно запустить так: ```docker-compose -f docker-compose-dev.yml -f docker-compose-dev-test.yml up --build```. После этого в Терминале нужно перейти в контейнер API: ```docker exec -it crud-api sh```. И там запустить тесты: ```npm run test``` или ```npm run test:e2e```.

## Технологии:
NestJS, TypeScript, Prisma, PostgresSQL
